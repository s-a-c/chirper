...
test('user password is automatically hashed when set', function (): void {
    ...
    expect($user->password)->not->toBe('plaintext-password')
        ->and(Hash::check('plaintext-password', $user->password))->toBeTrue();
});
...
test('user password is cast to hashed', function (): void {
    ...
    expect($user->password)->not->toBe('test-password')
        ->and(Hash::check('test-password', $user->password))->toBeTrue();
});
...
test('user can update name', function (): void {
    ...
    expect($fresh)->not->toBeNull()
        ->and($fresh)->toBeInstanceOf(User::class);
...
test('user can update password', function (): void {
    ...
    expect($fresh)->not->toBeNull()
        ->and($fresh)->toBeInstanceOf(User::class);
    assert($fresh instanceof User);
    expect($fresh->password)->not->toBe($oldPasswordHash)
...
test('user remember token is generated', function (): void {
    ...
    expect($user->remember_token)->not->toBeNull()
        ...
        ->and(mb_strlen($user->remember_token))->toBeGreaterThan(0);
});
...
test('user can be accessed as array', function (): void {
    ...
    expect($user['name'])->toBe('Array User')
        ->and($user['email'])->toBe('array@example.com');
});
...
test('user can check if attribute exists', function (): void {
    ...
    expect(isset($user->name))->toBeTrue()
        ->and(isset($user->email))->toBeTrue()
        ->and(isset($user->nonexistent))->toBeFalse();
});
...
test('user can be converted to string', function (): void {
    ...
    expect($string)->toBeString()
        ->and($string)->toContain('String User');
});
